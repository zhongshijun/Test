...
// 在处理过程中，涉及到的原始mic的数据都是缩小后的。
// 计算完回声估计后，记得放大同等比例的回声估计和误差估计!!!!
// 在送给滤波器前，先对MicIn原始输入信号做缩放*0.01，  (2023.05.03 MicIn不对吧？？ 不是参考信号吗？不然，做滤波有什么影响？？？？？！！)
for (i = 0; i < 257; ++i) {
  tmpSin[2 * i] = sin[2 * i] * 0.01;
  tmpSin[2 * i + 1] = sin[2 * i + 1] * 0.01;
}
// tmpSin 作为滤波器的实际输入值
// 注意！最后滤波器估计得到的回声估计也要放大回去；
for (i = 0; i < 257; ++i) {
  echoEst[2 * i] = echoEst[2 * i] * (1 / 0.01);
  echoEst[2 * i + 1] = echoEst[2 * i + 1] * (1 / 0.01);
}
// 最后的errEst是通过原始micIn和放大后的echoEst求得的。
...
// [1.1]. kalman filter
  ...
  smoothErrEstPow = SMOOTH(errEstPow, smoothErrEstPow, alhap)
...
// [1.3] kalman录波器发散判断
    ...
    // micPow: 缩小后的输入能量
    for (300Hz: 6800Hz) {
      errEstPowSum += errEstPow[i];
      errEstNLMSSum += errEstNLMS[i];
      micPowSum += micPow[i];
    }

    if (xdB > 10) {
      pwr = PowdB(micPowSum) - PowdB(errEstNLMSSum);
      pwr1 = PowdB(micPowSum) - PowdB(errEstPowSum);

      aec->cnt1 = pwr > 20 & pwr1 > 0 ? aec->cnt1 + 1 : 0;
      aec->cnt2 = pwr < 2 & pwr1 < -3 || pwr < 10 & pwr1 < -5 ? aec->cnt2 + 1 : 0;
      aec->diverfilterFlag = aec->cnt1 > 3 ? 1 : aec->diverfilterFlag;
      aec->diverfilterFlag = aec->cnt2 > 10 ? 0 : aec->diverfilterFlag;
    }
...
// [2.1] 根据kalman系数获取当前参考信号X
   ...
   // 需要对静音的ref信号随机增加白噪声
   for (i = 0; i < 96; ++i) {
       gtmp = ((cohxd[i] - cohxe[i]) * 1.5 + 1.0) * cohxe[i];
       gtmp = max(gtmp, cohxe[i]);
       gtmp = min(gtmp, 0.95); // 0.95(0~3K)/0.98(3~8K) 不同频域设置不同值，频带越大设置越大
       gtmp = 1.0 - gtmp;
       gain[i] = max(0.05, gtmp)
   }

   // 回声底噪估计
   for (i = 0; i < FilterBlockNum; ++i) {
    for (j = 0; j < 257; ++j) {
      errPow[j] = errPow[j] * kalman->w[i][j] * kalman->w[i][j];
    }
   }

   aec->EdB = PowdB(errPow, 300Hz, 800Hz);
   if (frame == 1) {
    GetNoiseEstimation(errPow, noies);
    aec->NdB = PowdB(noise, 300Hz, 800Hz);
   } else {
    aec->NdB = PowdB(noise, 300Hz, 800Hz);
    if (dVad =0 & pitch =0 || aec->NdB > aec->EdB - 2) {
      if (aec->xdB < 35 + 5) { // 35可配
        GetNoiseEstimation(errPow, noise);
      }
    }
   }

  if (aec->EdB - aec->NdB > 5) {
    aec->TNoise = true;
  } else {
    aec->TNoise = false;
  }

  if (aec->EdB - aec->NdB > 18) {
    aec->F0 = true;
  } else {
    aec->F0 = false;
  }
...
// [2.2] 基础信息计算
  ...
  // 掐头去尾计算cohedMean,coheecMean, cohxdMean, cohxd600HzMean; bandsize = 800Hz, [300Hz, 3800Hz]
   ...
   cohxdMean = tmpSum / (3800Hz - 800Hz + 1);
  ...
  for (300Hz: 300Hz + bandsize) {
   ...
   if (aec->coheecMean > 0.2 || cnt > 40) {
    ...
    aec->xbegin = max(0, aec->xbegin - 1);
    aec->xbegin1 = max(0, aec->xbegin1 - 1);

    if (aec->XdB < 10) {
      aec->muteCnt += 1;
    } else {
      aec->xbegin = (aec->muteCnt > 200) ? 20 : aec->xbegin;
      aec->xbegin1 = (aec->muteCnt > 2000 || aec->startFlag == 0) ?
                      20 : aec->xbegin1;
      aec->muteCnt = 0;
      aec->startFlag = 1;
    }
...
// 【3. 相关性回声估计】
  ...
  // 严格双讲判断
  if (aec->EdB - aec->NdB > 18 && aec->coheecMeanCnt == 0 || aec->EdB - aec->NdB > 18 && aec->DT >= 1) {
    aec->DTStrict = true;
  }
...
// 【4. 计算ERL, SER 回声估计】 
    ...
    if (ERLdB > ERLth & (aec->F0 == false || 近端vad == 0) & XdB > 10) {
       aec->STDR = true; // 严格单讲标志位
    }
    ...
    aec->SERCnt = alpha != 1 ? aec->SERCnt + 1 : aec->SERCnt;

    if (严格远端单讲) { // STDR1比STDR更加严格
        for (0:400Hz) {
 ...
 /////////////////////////[双讲场景回声估计系数调整]/////////////////////////////////////////////
    ...
    if (严格远端单讲 & 非双讲 & xdB > max(10, xdBsm- 10) & 非回声起始段) {
        F0 = 1200Hz;
        tmpPowSm = 0;
        for (1200Hz:6800Hz) {
            tmpPowSm[i]  = max(0, aec->se[i] - noiseEstimation[i]); // se:近端误差信号平滑能量
        }
        ...
        aec->alph1 = 0.2 * Max(5, aec->smoothdB - 35);
        ...
        aec->alph2 = 0.2 * Max(5, aec->smoothdB - 35);
        ...
        aec->alph3 = 0.2 * Max(5, aec->smoothdB - 35);
        ...
        aec->alph4 = 0.2 * Max(5, aec->smoothdB - 35);
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (i = 0; i < 257; ++i) {
      echoPow[i] = echoPow[i] * aec->afAlpha[i];
    }
    ...
    // TODO. 可以根据不同场景，单双讲，耳机场景的对回声估计进一步调整；可以使用的参数：
    // cohxdmeanCnt, cohxdmeansm, dtStrict,xbegin等；
...
for (i = 0; i < 257; ++i) {
    float tmp = 1;
    for(n = 0; n < numFilterBlock; ++n) {
        tmp *= kalman->ABDf[i][n];
    }
    gain[i] = min(tmp, gain[i]);
}
