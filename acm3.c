...
// 13.1 alpha1,2,3,4值计算

tmpPowSm = 0;
for (1200Hz:3600Hz) {
   tmpPowSm[i]  = refPowSmooth[i] - noiseEstimation[i];
}
refPowSmdB =  PowerdB(tmpPowSm, 1200Hz, 36000Hz);

// alph1 1200~2400Hz
refPowdB = PowerdB(refPow, 1200, 2400)
aec->alph1 = if (refPowdB > 10 ){
    if (当前近端-底噪dB > 上一帧平滑dB) {
      aec->smoothdB = 0.9 * aec->smoothdB + 0.1 * refPowSmdB;
    } else {
      aec->smoothdB = 0.99 * aec->smoothdB + 0.01 * refPowSmdB;
    }
...
alpha = 0.5 * SER_2sm // SER_2sm？？还是SER??
